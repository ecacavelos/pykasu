        ...
        try {
                ...
                */
                ...
                if (cell.getType().equals(Cell.CHECK)){
                    ...
                    if (cell.getData()!=null){
                        boolean checkValue = ((Boolean)cell.getData()).booleanValue();
                        if (checkValue)
                            cell.setData("Si");
                    }
            ...
            if (cell.getType().equals(py.com.roshka.pykasu.ui.form.Cell.NUMBER)) {
            	...
            	//value = "no tiene fecha";
            		value = "";
            	if (cell != null && (cell.getDatePattern() == null || cell.getDatePattern().trim().equals(""))){
            		cell.setDatePattern("dd/MM/yyyy");
            		
            	}
            	
            	if (cell.getData() != null && cell.getData() instanceof Calendar ){
            		
            		value = new java.text.SimpleDateFormat(cell.getDatePattern())
            					.format(new Date(((java.util.Calendar) cell.getData()).getTimeInMillis()));
            	}
            	else if (cell.getData() != null &&  cell.getData() instanceof Timestamp){
            		value = new java.text.SimpleDateFormat(cell.getDatePattern())
            					.format(new Date(((Timestamp) cell.getData()).getTime()));
            	}else if (cell.getData() != null &&  cell.getData() instanceof java.util.Date){
            		value = new java.text.SimpleDateFormat(cell.getDatePattern())
                    .format(cell.getData());
...
*/            	
            ...
            if (cell.getType().equals(py.com.roshka.pykasu.ui.form.Cell.LABEL)) {
                ...
                pw.print("<label align=\"" + align + "\">"
                    ...
                    py.com.roshka.pykasu.ui.form.Cell.CHECK)) {
                value = cell.getData().toString();
                ...
                pw.print(buff.toString());
                    ...
                    py.com.roshka.pykasu.ui.form.Cell.MULTIPLE)) {
                value = cell.getData().toString();
                ...
                if (cell.getEvents() != null
                    ...
                    for (int eventId = 0; eventId < events.length; eventId++) {
                        ...
                        String functions[] = events[eventId].split(":")[1].split("#");
                        ...
                        for (int functionIndex = 0; functionIndex < functions.length; functionIndex++) {
                            if (functionIndex > 0)
                                pw.print(", ");
                            if(!functions[functionIndex].contains("(")){ //si no tiene parametros al menos se le pasa (this). Si tiene parametros, se respera los que tiene, y NO se AGREGA this
                            	pw.print(functions[functionIndex] + "(this)");
                            }
                        }
                        if (eventType.toLowerCase().equals("onblur") && calculableFunction!=null && calculableFunction.length()>0){
                ...
                for (int choice = 0; choice < choices.length; choice++) {
                    ...
                    if (choices[choice].equals(value) || ((value == null || value.equals("") && choice == 0)))
                 ...
                 * pw.print("<label
                 * style=\"font-size:8px\">"+cell.getAdditionalDescription()+"</label>"); }
                 */
                if (!cell.isOpaque()){
                ...
                try {
                    if (cell.getEvents() != null && cell.getEvents().length() > 0) {
                        ...
                        for (int eventId = 0; eventId < events.length; eventId++) {
                            ...
                            if (eventType.toLowerCase().equals("onblur") && calculableFunction!=null && calculableFunction.trim().length()>0){
                                pw.print(","+calculableFunction);
                            }
                            pw.print("\"");
                        }
                ...
                pw.print(">");
                    ...
                    py.com.roshka.pykasu.ui.form.Form form = (py.com.roshka.pykasu.ui.form.Form) super.pageContext.getRequest()
                    	.getAttribute(py.com.roshka.pykasu.web.Globals.FORM_BEAN);
                    pw.print("<img class=\"opaque\" src=\"images/hidden" + form.getId() + ".jpg\">");
                }
            }
        } catch (Exception e) {
